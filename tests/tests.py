...
# Импорты в зависимости от вашей структуры
try:
    # Если у вас есть пакет csv_filter
    from csv_filter.core.csv_filter import CSV_Filter
    from csv_filter.main import main
except ImportError:
    # Если main.py в корне и классы в нем
    from main import CSV_Filter, main
...
def load_test_data_mock():
    ...
    file_path = os.path.join(os.path.dirname(__file__), 'tables.csv')
...
def test_main_file_not_found(capsys):
    ...
    with patch('sys.argv', test_args), \
         ...
         pytest.raises(SystemExit):  # Ожидаем выход с ошибкой
        main()
